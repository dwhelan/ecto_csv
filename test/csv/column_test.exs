defmodule CSV.ColumnTest do
  ...
  describe "transform function" do
    test "as a String with module" do
      ...
      assert Column.transform("123", to_int) === {:ok, 123}
    end
  ...
  describe "transform module" do
    ...
    test "as the first module in options[:modules]" do
      assert Column.transform("123", :to_int, modules: [CSV.ColumnTest, String]) === {:ok, 123}
    end

    test "as the last module in options[:modules]" do
      assert Column.transform("123", :to_int, modules: [String, CSV.ColumnTest]) === {:ok, 123}
    end

    test "as the only module in options[:modules]" do
      assert Column.transform("123", :to_int, modules: [CSV.ColumnTest]) === {:ok, 123}
    end
  ...
  test "should ignore extra options" do
    assert Column.transform("123", &to_int/1, foo: "bar") === {:ok, 123}
  end
end
